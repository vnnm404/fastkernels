...
def hub_retry(call, attempts=4):
    ...
    for attempt in range(attempts):
        try:
            ...
            try:
                import httpx
                connection_error |= isinstance(exc, (httpx.TimeoutException, httpx.NetworkError, httpx.RemoteProtocolError))
            except ImportError:
                pass
            try:
                import requests
                connection_error |= isinstance(exc, (requests.exceptions.Timeout, requests.exceptions.ConnectionError))
            except ImportError:
                pass
            if (status not in (429, 500, 502, 503, 504) and not connection_error) or attempt == attempts - 1:
                raise
            try:
                delay = float(getattr(response, 'headers', {}).get('Retry-After', 2 ** (attempt + 1)))
...
def access_problem(exc):
    ...
    if status in (401, 403):
        return {'status': 'blocked', 'reason':
                f'Hugging Face HTTP {status}: verify HF_TOKEN/HF_TOKEN_PATH and '
...
def probe(config_dir):
    ...
    dependencies = ['vllm', 'transformers', 'torch', 'datasets', 'pyarrow', 'fastsafetensors']
    if config.get('model_type') == 'whisper' or any(k in config for k in ('vision_config','audio_config','thinker_config')):
    ...
    for name in dependencies:
        try:
            ...
            return dict(result, status='environment-error', reason=f'{name} import failed: {type(exc).__name__}: {exc}')
    ...
    try:
        ...
        if architectures and not native:
            return dict(result, status='unsupported', reason=
                        f'vLLM {result["packages"]["vllm"]} has no native implementation for {architectures}, '
                        f'and its installed config loader failed: {type(exc).__name__}: {exc}')
        return dict(result, status='environment-error', reason=f'Config cannot load in this reference: {type(exc).__name__}: {exc}')
    if architectures and not native:
        ...
        result['warning'] = f'No native registry entry for {architectures}; vLLM auto backend must resolve this model at initialization.'
    return result
