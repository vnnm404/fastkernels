...
from fastkernels.validate import compatibility, drift_prepare as suite
from fastkernels.validate.drift_results import pair_metrics
...
class CompatibilityTests(unittest.TestCase):
    ...
    def test_preflight_blocks_download_and_can_resume_after_access_fixed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config.write_text('{"architectures":["Gemma4ForConditionalGeneration"]}')
            metadata = root / "model.json"
            hub = SimpleNamespace(
                HfApi=lambda: SimpleNamespace(
                    model_info=lambda *a, **kw: SimpleNamespace(
                        sha="frozen",
                        siblings=[
                            SimpleNamespace(size=10, rfilename="model.safetensors")
                        ],
                    )
                ),
            ...
            with patch.dict(sys.modules, {"huggingface_hub": hub}):
                ...
                hub.hf_hub_download.side_effect = None
                hub.hf_hub_download.return_value = str(config)

                def unsupported(command, *args, **kwargs):
                    Path(command[-1]).write_text(
                        json.dumps(
                            {"status": "unsupported", "reason": "Architecture absent"}
                        )
                    )
                ...
                with patch.object(suite.subprocess, "run", side_effect=unsupported):
                    suite.prepare_model(
                        ...
                        root,
                        0,
                        interpreters={"baseline": "/python"},
                    )
                self.assertEqual(
                    json.loads(metadata.read_text())["status"], "unsupported"
                )
                hub.snapshot_download.assert_not_called()

                def supported(command, *args, **kwargs):
                    Path(command[-1]).write_text(json.dumps({"status": "ok"}))
                    return SimpleNamespace(returncode=0)
                ...
                self.assertEqual(
                    hub.snapshot_download.call_args.kwargs["revision"], "frozen"
                )
